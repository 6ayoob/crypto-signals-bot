...
# متوافق مع bot.py V2 + Leader Lock (TTL) — مضاف updated_at لجدول users
import os
import logging
...
class User(Base):
    ...
    # جديد: لضمان التوافق مع جداول موجودة عليها NOT NULL
...
def _add_column_sql(table: str, col: str, dialect: str) -> str:
    ...
    return f'ALTER TABLE "{table}" ADD COLUMN "{col}" {typ};'  # SQLite لا يدعم IF NOT EXISTS مع الأعمدة
...
def _lightweight_migrate():
    ...
    with engine.begin() as conn:
        ...
        try:
            ...
            cols_users = set()
        required_users = ["tg_user_id", "trial_used", "end_at", "plan", "last_tx_hash", "created_at", "updated_at"]  # NEW
        ...
        # في Postgres: عيّن DEFAULT و NOT NULL إن لم تكن مضبوطة (لأمان إضافي)
        if dialect == "postgresql":
            try:
