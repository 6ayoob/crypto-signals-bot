...
def check_signal(
    ...
    try:
        ...
        hr_riyadh = 12
    ok_qv, qv_dbg = _qv_gate(qv_series, float(prof["min_quote_vol"]),
                             win=10, low_vol_env=low_vol_env,
                             is_major=is_major, hr_riyadh=hr_riyadh)
    if not ok_qv:
    ...
    if (
        breakout_ok and retest_ok and not brk_far
        and (rvol >= thr["RVOL_MIN"] or spike_ok) and brk_in_session
    ):
    ...
    # SL وأهداف (مع قصّ T1 تحت المقاومة)
    ...
    return {
        ...
        "partials": partials,
        "trail_after_tp2": TRAIL_AFTER_TP2, "trail_atr_mult": (0.9 if score>=88 else 1.0 if score>=84 else 1.2 if score>=76 else 1.4) if TRAIL_AFTER_TP2 else None,
