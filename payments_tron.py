...
TRONGRID_API_KEY = os.getenv("TRONGRID_API_KEY")  # مفتاح TronGrid (مستحسن)
...
def get_tx(txid: str):
    ...
    r.raise_for_status()
    data = r.json()
    # استجابات TronGrid غالباً في data[]
...
def find_trc20_transfer_to_me(txid: str, min_amount: float) -> tuple[bool, str]:
    ...
    # قد تعيد TronGrid قائمة trc20TransferInfo داخل الكائن
    trc20_list = tx.get("trc20TransferInfo") or tx.get("tokenTransferInfo") or []

    if not trc20_list:
        # أحياناً تحتاج endpoint آخر لاستخراج الأحداث، نبقيها بسيطة الآن
    ...
    for ev in trc20_list:
        ...
        # تقييد بالـ USDT: إما via symbol أو بالعقد إذا وضعته
        if USDT_CONTRACT:
            if contract != USDT_CONTRACT:
                continue
        else:
            # لو لم تضع عقدًا: نتحقق بالرمز إن توفر
            if symbol and symbol != "USDT":
                continue

        # يجب أن يكون التحويل إلى محفظتنا
